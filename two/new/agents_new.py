...
class Agent(object):
	...
	def off_mc(self, visited_pairs, returns, discount_factor):
		""" Computes Q-values using off-policy Monte Carlo """
		#Store steps needed in this episode
		amount_rounds = len(visited_pairs)
		#For each pair s,a in the episode
		for pair in visited_pairs:
			#Initialize weight w at 1
			pair_w = 1
			#Compute last time the behavioral policy took a different action than the greedy policy
			pair_t = self.N_policy.get_N_policy(pair[0])[pair[1]][0]
			#For all state,action pairs in the episode after pair_t (aka following greedy policy)
			for i in range(pair_t, amount_rounds-1):
				#Store the state,action pair
				k_pair = visited_pairs[i]
				#Get the state's policy
				policy = self.N_policy.get_policy(k_pair[0])
				#Compute the probability of the state,action pair according to the policy
				current_prob = self.N_policy.get_e_greedy_policy(policy, epsilon=1.0)[k_pair[1]]
				#Update the weight
				pair_w+=(1.0/current_prob)
			#Compute return
			power = amount_rounds - pair_t
			discounted_cumulative_reward = 10 * (discount_factor**power)
			#Get current nominator and denominator
	...
	def update_returns(self, pairs_list, cumulative_reward, discount_factor):
		""" Updates the returns list for each state,action pair"""
